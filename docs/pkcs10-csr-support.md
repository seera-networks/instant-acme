...
- `rustls-pki-types`（**必須依存**、現在 `"1.1.0"` 指定）に `CertificateSigningRequestDer<'a>` と `pem::PemObject` があり、`from_pem_slice` / `from_pem_file` が使える（1.15.0 および 1.12.0 のソースで確認済み。正確な導入バージョンは crates.io で要確認、最低 1.12 への引き上げを想定）。PEM 対応は依存追加ゼロで実現できる。
...
3. `rcgen` feature を無効にしても（＝instant-acme が鍵生成コードを一切含まない構成でも）、実運用の完全なワークフローがサンプルとドキュメントで示されている。
...
`Csr` に検証メソッドを実装する。`x509-parser` feature 有効時のみ。

```rust
#[cfg(feature = "x509-parser")]
impl Csr<'_> {
    /// CSR がオーダーの identifier をすべてカバーしているか検査する
    pub fn validate(&self, identifiers: &[Identifier]) -> Result<(), CsrError>;
}
```
...
5. **アカウント鍵の再利用禁止**（RFC 8555 §11.1）— CSR の SPKI がアカウント鍵の公開鍵と一致したらエラー。これは検証の中でも実利が大きい（やらかすと CA から拒否され、原因が分かりにくい）。
...
let mut it = order.identifiers();
while let Some(res) = it.next().await { idents.push(res?.identifier.clone()); }

csr.validate(&idents)?;              // ネットワーク往復なしで事前検査
order.finalize_with(&csr).await?;
```

エラー型は `Error` に新しい variant を足すか（`Error` は `#[non_exhaustive]` かどうか確認のうえ判断）、`Error::Other(Box<CsrError>)` に包む。既存の `Error::from_rcgen` と同じ書き方（`src/types.rs:68`）に倣うなら後者が最小変更。ただし呼び出し側が原因別に分岐したいはずなので、`CsrError` を公開型として定義し `Error::Csr(CsrError)` を追加する案を推す。
...
struct KmsKey { spki_der: Vec<u8>, handle: KmsHandle }

impl rcgen::PublicKeyData for KmsKey {
    fn der_bytes(&self) -> &[u8] { &self.spki_der }               // SPKI の BIT STRING 中身
...
order.finalize_with(&Csr::from_der(csr.der())).await?;
```
...
```bash
# 鍵は一度もアプリケーションに渡らない
openssl req -new -key /dev/stdin -subj "/" \
...
| `src/order.rs` | `finalize_with()` の追加、`finalize_csr()` を薄いラッパ化、doc コメントに CSR 経路の説明追加 | 1 |
| `src/lib.rs` | `pub use types::{Csr, CsrError}` の追加 | 1 |
| `Cargo.toml` | `rustls-pki-types` 最低バージョンの引き上げ、`aws-lc-rs`/`ring` feature から `x509-parser?/verify-aws`・`x509-parser?/verify` への伝播、新 example の登録 | 1, 2 |
...
- [ ] PEM ラウンドトリップの unit テスト
- [ ] `cargo build --no-default-features --features hyper-rustls,aws-lc-rs` など主要 feature 組み合わせでビルド確認（`rcgen` 無効でも CSR 経路が使えることの回帰テスト）

### PR 2: 検証（フェーズ 2）

- [ ] `CsrError` と `Csr::validate()` を実装（`x509-parser` feature）
...
| CSR の公開鍵 == アカウント鍵 | `CsrError::AccountKeyReuse` |
...
- **semver**: 案 C を採る限り PR 1 と PR 3 は純粋な追加。PR 2 も `Error` に variant を足す点だけが破壊的（`Error` が `#[non_exhaustive]` なら非破壊）。0.9.x のマイナー追加、または 0.10.0 でまとめて出す。
- **MSRV**: 現在 1.85。新しい言語機能は不要なので据え置き。
- **依存**: 追加なし。`rustls-pki-types` の最低バージョン引き上げのみ。バイナリサイズ・ビルド時間への影響は無視できる。
- **feature 表**（更新後の想定）

  | feature | CSR 経路への影響 |
  | --- | --- |
  | （なし） | `Csr::from_der` / `from_pem` / `finalize_with` が使える。鍵生成コードは一切リンクされない |
  | `x509-parser` | `Csr::validate()` が使える |
...
1. **検証を暗黙にするか明示にするか** — 本書は明示（`validate()` を呼ぶのは利用者）を推奨。feature unification による挙動変化を避けるため。上流の意見次第では、`finalize_with` に `CsrPolicy` 引数を持たせる案（デフォルトで検証あり、feature 無効時はコンパイルエラーではなく検証スキップ）も検討余地あり。
...
4. **`x509-parser` の `verify` feature が `ring` を引き込む** — instant-acme が `aws-lc-rs` のみ構成のとき、誤って `verify`（ring 版）を有効にすると ring が余計にリンクされる。feature マッピングを間違えないこと。テストで `cargo tree` を確認する。
